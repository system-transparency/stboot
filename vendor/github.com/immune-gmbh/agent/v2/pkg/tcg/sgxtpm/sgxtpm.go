...
//
// This file implements client code to communicate with server code described
...
const (
	...
	// immune extensions for SGX
	tpmGetPlatformId uint32 = 0xCAFE0
	tpmGetSgxQuote   uint32 = 0xCAFE1

	// SGX length constants
	sgxPlatformIdLen     uint32 = 32
	sgxUserReportDataLen uint32 = 64
	sgxQuoteMaxLen       uint32 = 8192
)
...
func (c *Conn) RequestPlatformId() ([]uint8, error) {
	return c.requestSgxCmdResponse(tpmGetPlatformId, nil, sgxPlatformIdLen)
}

// send an SGX TPM quote request
func (c *Conn) RequestSgxQuote() ([]uint8, error) {
	userReportData := make([]uint8, sgxUserReportDataLen)
	return c.requestSgxCmdResponse(tpmGetSgxQuote, userReportData, sgxQuoteMaxLen)
}

// send an SGX TPM command request that retrieves data
func (c *Conn) requestSgxCmdResponse(u uint32, sendData []uint8, rlen uint32) ([]uint8, error) {
	if err := binary.Write(c.conn, binary.BigEndian, u); err != nil {
		return nil, fmt.Errorf("write SGX tpm command: %v", err)
	}

	if sendData != nil {
		if err := binary.Write(c.conn, binary.BigEndian, sendData); err != nil {
			return nil, fmt.Errorf("write SGX tpm command: %v", err)
		}
	}

	response := make([]uint8, rlen)
	if err := binary.Read(c.conn, binary.BigEndian, &response); err != nil {
		return nil, fmt.Errorf("read SGX tpm response: %v", err)
	}
	...
	if rc != 0 {
		return nil, fmt.Errorf("unexpected SGX tpm command response code: 0x%x", rc)
	}
...
func (c *Conn) Read(b []byte) (int, error) {
	...
	if err := binary.Read(c.conn, binary.BigEndian, &respLen); err != nil {
		return 0, fmt.Errorf("read SGX tpm response header: %v", err)
	}

	resp := make([]byte, int(respLen))
	if _, err := io.ReadFull(c.conn, resp[:]); err != nil {
		return 0, fmt.Errorf("read SGX tpm response: %v", err)
	}

	var rc uint32
	if err := binary.Read(c.conn, binary.BigEndian, &rc); err != nil {
		return 0, fmt.Errorf("read SGX tpm return code: %v", err)
	}
	if rc != 0 {
		return 0, fmt.Errorf("SGX tpm returned invalid return code: 0x%x", rc)
	}
...
func (c *Conn) Write(b []byte) (int, error) {
	...
	if _, err := buff.WriteTo(c.conn); err != nil {
		return 0, fmt.Errorf("write SGX tpm command: %v", err)
	}
	return len(b), nil
}
